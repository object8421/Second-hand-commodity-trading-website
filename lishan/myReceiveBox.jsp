...
<div align=left>
<form name=form1 method=post target=_self onsubmit="checkDel()">
<input type=hidden name=toURL value=myReceiveBox.jsp>
...
已收邮件列表
	...
	<%
		sendMail.receiver = ((com.netshop.user)session.getAttribute("user")).userName;
		ResultSet rs = sendMail.emailList();
		while(rs.next())
		{
		%>
			<tr>
			...
			<%
				if(rs.getInt("tag") == 0)
				{
					out.println("<font color=red>new!</font>");
			     }
			%>
				
			</td>
			<td align=left>
			<%=codeToString(rs.getString("sender"))%>
			</td>
			<td align=left>
