...
<%
	...
	goods.init(strID);
	goodsField.init(String.valueOf(goods.goodsField));
...
<TABLE cellSpacing=1 cellPadding=3 width="90%" align=center   border=0>
  <FORM name="form1" onReset="return confirm('确定要清空当前表单吗？');" 
        onsubmit="return infocheck();" action=goodsModifySave.jsp method=post enctype="multipart/form-data" >
		<input type=hidden name="ID" value=<%=strID%> >
        <TBODY>
        <TR >
          <TD align=right >商品名称：</TD>
          <TD align=left >
		  <input type="text" name="title" size="20" value=<%=codeToString(goods.title)%>>
		  </TD>
		  </TR>
        <TR  >
          <TD align=right >商品类别：</TD>
          <TD align=left >
			<select style="WIDTH: 150px" id=goodsField name="goodsField" >
                 <%
							String strFields = String.valueOf(goods.goodsField);
